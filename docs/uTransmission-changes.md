...
База: Transmission `a89f4bd5b`. Наши изменения лежат несколькими коммитами поверх неё.
Remote'ы: `origin` — наш репозиторий `github.com/whiterabbit74/uTransmission`,
`upstream` — апстрим `github.com/transmission/transmission`.
...
```sh
git fetch upstream
git rebase upstream/main        # или upstream/<нужный тег>
./code_style.sh                 # pre-commit хук требует clang-format
```

`README.md` в корне заменён на наш и будет конфликтовать при каждом обновлении
апстрима — берите нашу версию и при необходимости переносите в неё изменения по сути.
