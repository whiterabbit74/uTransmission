...
4. Download `binaries-windows-x64-msi` and run the installer inside it.

Downloading artifacts requires being signed in to GitHub — an account is enough, no permissions on
the repository are needed.

Take the MSI rather than the `binaries-windows-x64` artifact: the latter is only the build's install
prefix, so its `transmission-qt.exe` has no Qt DLLs beside it and will refuse to start. The
installer ships the Qt runtime, its plugins and the translations, and puts a `uTransmission`
...
the other. `View → Sidebar` and the leftmost toolbar button both hide and show it, and stay in sync
with each other; the divider between the sidebar and the list can be dragged.
...
**Naming.** The window title and taskbar entry say `uTransmission`, and the executable, the
installer and the shortcuts carry the fork's name and icon. The names of the executable itself and
of the configuration directory are intentionally left alone.
